...
\begin{itemize}
\item Test Item % Replace the entire original content with this simple text
\end{itemize}
%
...
\end{enumerate}
To ensure reproducibility of evaluation results, accurate documentation and adjustment of these parameters are important.
...
\begin{itemize}
    \item \textbf{API-based Inference:} Through a so-called \ac{api}s, language models that are provided and hosted by third-party providers (such as GPT-4 from OpenAI, which \citet{Paul_Keller} referenced as a benchmark) can be accessed.
...
\section{Evaluation Metrics for Knowledge Retrieval and Generation}\label{sec:evaluation_metrics}

Meaningfully assessing the performance of \ac{llm}s in knowledge-based tasks, such as answering questions using \citet{bb2} as a source, requires the use of well-defined evaluation metrics. 
Given that a primary goal of this thesis is to provide evaluations comparable to the benchmark established by \citet{Paul_Keller}, it is essential to understand the metrics employed in that study along with other relevant standard measures.

\subsection{Standard Retrieval and Classification Metrics}
Although the final output of a \ac{lm} is a generated text, the underlying retrieval processes, as in \ac{rag} and the evaluation of the correctness and accuracy of the generated response often rely on standard metrics from \ac{ir} and classification tasks:
\begin{itemize}
    \item \textbf{Precision:} \textit{Precision} is defined as the proportion of retrieved or generated facts within an answer that are actually correct in relation to the ground truth.
    It measures the accuracy of the generated information.
    In fields such as medicine, where scientific and factual correctness and completeness are always strived for, maximising precision is often prioritised, as highlighted by\citet{Paul_Keller}:\\
    \( \text{Precision} = \frac{|\text{Correct Items}|}{|\text{Generated Items}|} \).
    \item \textbf{Recall:} Recall measures the completeness of the generated information.
...
\( \text{Recall} = \frac{|\text{Correct Items}|}{|\text{Expected Correct Items}|} \).
    \item \textbf{F1-Score:} Represents the harmonic mean of precision and recall, offering a single, balanced measure of performance:\\ \( F_1 = 2 \times \frac{\text{Precision} \times \text{Recall}}{\text{Precision} + \text{Recall}} \).
    \item \textbf{Macro-F1 vs. Micro-F1:} 
    When evaluating the perfomance of a system usually sets of queries are used for the assessment of continous und overall quality of output.
    Therefore, individual \textit{F1-scores} need to be accumulated and calculated into an over all average.
    Two main methods exist for this aggregation:\\
    To create the \textit{Micro-average/Micro F1}, all individual correctly generated and expected items are collected together across all questions and finally a uniting \textit{Micro F1-Score} is calculated.\\
    \textbf{Macro-averaging} first determines the \textit{F1-score} for each question of the set and then calculates the \textit{Macro F1-score} by averaging these individual question scores.
   ...
   This approach ensures equal weight proportioning of individual often divers queries.
\end{itemize}

\subsection{Metrics for Large Language Model Generation Quality (Benchmarked)}
Evaluating the nuanced quality of generated text often requires metrics that go beyond simple fact counting. Based on the benchmark study by \citet{Paul_Keller} and common practices in LLM evaluation, pertinent quality dimensions include:
\begin{itemize}
    \item \textbf{Correctness:} The fundamental assessment of factual accuracy against a reliable source (\citet{bb2}) or expert consensus. \citet{Paul_Keller} implemented this through manual comparison, classifying each generated answer as Correct (Richtig), False (Falsch), or Not Answered (Nicht beantwortet). 
    This forms a core part of the evaluation necessary for \cref{def:goal 4}.
    \item \textbf{Explainability:}\label{def:explainability} The extent to which the model provides plausible and appropriate reasoning or argumentation within the answer.
    \citet{Paul_Keller} has provided a manual evaluation of this quality.
Due to the focus on the medical domain, the importance of this measure must be emphasised even more. 
Relevant knowledge in medical informatics is often complex, comprehensive and multifaceted.
Therefore, the ability to make the extracted and accumulated knowledge understandable to the user is crucial in the evaluation of the language model and the generated answer.
    \item \textbf{Robustness:} The model's resilience to minor perturbations in the input, such as typos or grammatical errors. 
    The model's resilience to minor disruptions in the input.
    In the a real world application scenario, human users will not be able to provide ideal query inputs without corrective prompting. 
    Grammar errors and typos may occur. 
    \citet{Paul_Keller} tested this by evaluating performance on queries where such mistakes were deliberately introduced.
    \item \textbf{Human Evaluation:} Judgements about attributes such as scientific correctness, fluency, coherence, relevance, conciseness and general usefulness often rely on human judgement.
    This provides a gold standard assessment that complements automated metrics.
    An expert assessment is particularly valuable for assessing the practical usability of the generated answers, for example, in the context of this work, for the evaluation of exam questions \cref{def:goal 4}.
\end{itemize}
The use of these metrics, in particular the equivalent application of the methodology of \citet{Paul_Keller}, which is set as a benchmark, is of central importance for the goal of this work – to provide a standardised and comparable evaluation of LLM performance for knowledge extraction from \citet{bb2}.
%
