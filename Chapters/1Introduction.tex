...
%
To ensure that these possibilities for knowledge retrieval and processing using Large Language Models provide and meet individual requirements, such as reliability and efficiency, a structured, repeatable, and reproducible evaluation is necessary. 
...
%
Furthermore, in the application of \ac{llms} for knowledge retrieval from \citet{bb2}, an automatic, objective and repeatable evaluation and further comparison to a benchmark of the generated results is necessary, but not available to users, such as students and practitioners. 
%
Key problem:
\begin{itemize}
  ...
  \item \textbf{Problem 2:} Existing \ac{llm}-based retrieval approaches for \citet{bb2} as a knowledge base lack a standardised and reproducible evaluation framework.
\end{itemize}
%
...
Building on this, \citet{hannesbell, hannesbell_skill} in the context of a \ac{bell} integrated natural language processing and implemented the results in a \ac{qas} QAnswer ~\citep{qanswer}, though showing deficits in the explainability and comprehensibility of more complex questions. 
...
Individually fine-tuning an open-source \ac{llm}, such as Llama-2, has demonstrated an improved performance over the untrained version, but still shows significant performance differences compared to more capable language models, such as OpenAI's GPT-4 \citep{Paul_Keller}.

In conclusion, \citet{Paul_Keller} suggests to leverage more capable language models, such as current commercially available models (e.g., GPT-4 / ChatGPT, Gemini-1.5 / Google Gemini) to improve and overcome these deficiencies in knowledge extraction and processing.
In addition, more advanced techniques such as \ac{rag} in conjunction with \ac{llm}s hold even greater potential, allowing further refinement of factual accuracy, improvement of intuitive extraction and optimisation of automatic answer generation beyond the inherent strengths of these models.\\
%
