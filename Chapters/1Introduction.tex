...
\todo{Ich habe Resource-limited nochmal expliziter aufgenommen. "Retrieval model" geht für mich an dieser Stelle noich weiter als die reinen LLM´s, also auch die Art und Weise, wie man diese verwenden kann, also auch die Anwendung (App, RAG, etc.). 
Ich habe es nun aber doch umformulier }
The key challenge is to find an application model that ensures accessibility and availability at all times and in any application scenario, while solving the problem of fragmented knowledge and scientific and factual reliability.
\todo{Dieser Satz fokussiert sich nun auf die Problematik, die ich lösen möchte. }
The need for this application exists above all for resource-limited target groups. 
The student who needs explicit domain-specific information from selected sources quickly and easily in preparation for an exam or when working on an exercise. 
The teacher who wants to prepare a lecture, discussion or exam and does not have time to look up complex contexts or details from a source.
...
Entschuldige, die Sätze sind mir beim Schreiben verrutscht und ich hatte die geänderten Sätze vor dem Pushen nicht alle Zeile für Zeile getrennt. Sonst achte ich darauf. }
