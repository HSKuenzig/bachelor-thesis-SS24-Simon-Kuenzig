\begin{acronym}[SPARQL]
% A
\acro{ai}[AI]{Artificial Intelligence}
\acro{api}[API]{Application Programming Interface}
% B
\acro{bart}[BART]{Bidirectional and Auto-Regressive Transformers}
\acro{bell}[BeLL]{Besondere Lernleistung}
\acro{bert}[BERT]{Bidirectional Encoder Representations from Transformers}
\acro{bpe}[BPE]{Byte-Pair Encoding}
% C
\acro{cot}[CoT]{Chain-of-Thought}
% D
\acro{db}[DB]{Database}
\acro{dbms}[DBMS]{Database Management System}
\acro{dpr}[DPR]{Dense Passage Retrieval}
% E
% F
% G
\acro{gpt}[GPT]{General Pretrained Transformer}
\acro{gui}[GUI]{Graphical User Interface}
% H
\acro{him}[HIM]{hospital information systems and their management}
\acro{his}[HIS]{hospital information systems}
\acro{hl7v2}[HL7 V2]{Health Level 7 Version 2}
% I
\acro{ir}[IR]{Information retrieval}
% J
% K
% L
\acro{lcw}[LCW]{Large Context Window}
\acro{llama}[LLaMA]{Large Language Model Meta AI}
\acro{llm}[LLM]{Large Language Model}
\acro{lcllm}[LC-LLM]{Long-Context Large Language Model}
\acro{lm}[LM]{Language Model}

\acro{longrope}[LongRoPE]{Long Rotary Position Embeddings}
% M
% N
\acro{nlp}[NLP]{Natural Language Processing}
\acro{nosql}[NoSQL]{Not Only SQL}
\acro{nn}[NN]{Neural Networks}
% O
% P
\acro{pose}[PoSE]{Positional Skip-wisE}
% Q
\acro{qa}[QA]{Question Answering}
\acro{qas}[QAS]{Question Answering System}
% R
\acro{rdf}[RDF]{Resource Description Framework}
\acro{rag}[RAG]{Retrieval-Augmented Generation}
\acro{rope}[RoPE]{Rotary Position Embeddings}
% S 
\acro{sota}[SOTA]{state-of-the-art}
\acro{sqa}[SQA]{Semantic Question Answering}
\acro{sql}[SQL]{Structured Query Language}
\acro{snik}[SNIK]{Semantisches Netz des integrierten Informationsmanagements im Krankenhaus}
% T
% U
\acro{ui}[UI]{User Interface}
% V
% W
% X
% Y
\acro{yarn}[YARN]{Yet another RoPE extensioN method}
% Z
\end{acronym}
\acused{URL}% Has its own paragraph in the preliminaries.
